...
fQCD = TFile('TWPileupQCD.root','open')
...
leg5 = TLegend(0.7,0.7,0.93,0.9)

hQCDW = hQCD.Clone('QCDW')
hQCDW.Multiply(hratio)
hQCDW.SetLineColor(kBlack)
hQCDW.SetTitle('')
hQCDW.GetXaxis().SetTitle('NPV')
hQCDW.GetYaxis().SetTitle('Fraction')

leg5.AddEntry(hQCDW, 'Weighted QCD MC', 'L')
leg5.AddEntry(hQCD, 'Unweighted QCD MC', 'L')

hQCDW.Draw('samehistE')
