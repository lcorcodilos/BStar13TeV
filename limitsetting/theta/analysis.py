...
def get_model_semileptonic():
		files = ['BStarCombinationHistos_Vector_Semileptonic.root']
...
def get_model_hadronic():
	files = ['BStarCombinationHistos_Vector_Allhadronic.root']
	model = build_model_from_rootfile(files, histogram_filter, external_to_internal, include_mc_uncertainties=True)
	model.fill_histogram_zerobins()
	model.set_signal_processes('bs*')
	for p in model.processes:
...
def get_model_dileptonic():
		#files = ['HistoForShapeLimitbStarDileptonVector.root']
		files = ['BStarCombinationHistos_Vector_Dileptonic.root']
...
def limits_comb(model, step = 1):
	...
	plot_expected.write_txt('expected_hadronic_semileptonic_limits_Vector.txt')
	plot_observed.write_txt('observed_hadronic_semileptonic_limits_Vector.txt')
	...
	write_histograms_to_rootfile(histos, 'Vector_hisos_at_ML.root')

	saveout = sys.stdout
	Outf1   =   open("Vectornuisance.txt", "w")
	sys.stdout = Outf1
	print parVals
	sys.stdout = saveout


	report.write_html('htmlout_Vector_hadronic_semileptonic')
def limits_semilep(model, step = 1):
	...
	plot_expected.write_txt('expected_semileptonic_limits_Vector.txt')
	plot_observed.write_txt('observed_semileptonic_limits_Vector.txt')
	...
	report.write_html('htmlout_Vector_semileptonic')
def limits_allhad(model, step = 1):
	...
	plot_expected.write_txt('expected_hadronic_limits_Vector.txt')
	plot_observed.write_txt('observed_hadronic_limits_Vector.txt')
	...
	report.write_html('./htmlout_Vector_hadronic')
...
def limits_dilep(model, step = 1):
	...
	plot_expected.write_txt('expected_dileptonic_limits_Vector.txt')
	plot_observed.write_txt('observed_dileptonic_limits_Vector.txt')

	report.write_html('htmlout_Vector_dileptonic')
