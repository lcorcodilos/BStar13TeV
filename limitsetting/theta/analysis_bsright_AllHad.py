...
def get_model_semileptonic():
    files = ['BStarCombinationHistos_Right_Semileptonic.root']
...
def get_model_hadronic():
    files = ['BStarCombinationHistos_Right_Allhadronic.root']
    model = build_model_from_rootfile(files, histogram_filter, external_to_internal, include_mc_uncertainties=True)
    model.fill_histogram_zerobins()
    model.set_signal_processes('bs*')
    for p in model.processes:
	...
	if p=='ttbar':
      ...
      # 	model.add_lognormal_uncertainty('topsf', math.log(1.15), p)
        ...
        model.add_lognormal_uncertainty('wtag', math.log(1.029), p)
...
def get_model_dileptonic():
    #files = ['HistoForShapeLimitbStarDileptonRight.root']
    files = ['BStarCombinationHistos_Right_Dileptonic.root']
...
def limits_comb(model, step = 1):
   ...
   plot_expected.write_txt('expected_hadronic_semileptonic_limits_Right.txt')
   plot_observed.write_txt('observed_hadronic_semileptonic_limits_Right.txt')
   ...
   write_histograms_to_rootfile(histos, 'Right_hisos_at_ML.root')

   saveout = sys.stdout
   Outf1   =   open("Rightnuisance.txt", "w")
   sys.stdout = Outf1
   print parVals
   sys.stdout = saveout


   report.write_html('htmlout_Right_hadronic_semileptonic')
def limits_semilep(model, step = 1):
   ...
   plot_expected.write_txt('expected_semileptonic_limits_Right.txt')
   plot_observed.write_txt('observed_semileptonic_limits_Right.txt')
   ...
   report.write_html('htmlout_Right_semileptonic')
def limits_allhad(model, step = 1):
   ...
   plot_expected.write_txt('expected_hadronic_limits_Right.txt')
   plot_observed.write_txt('observed_hadronic_limits_Right.txt')
   ...
   report.write_html('./htmlout_Right_hadronic')
...
def limits_dilep(model, step = 1):
   ...
   plot_expected.write_txt('expected_dileptonic_limits_Right.txt')
   plot_observed.write_txt('observed_dileptonic_limits_Right.txt')

   report.write_html('htmlout_Right_dileptonic')
