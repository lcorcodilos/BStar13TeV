...
if jobs != 1:
	f = TFile( "TWrootfile"+options.set+"_job"+options.num+"of"+options.jobs+"_PSET.root", "recreate" )
else:
	f = TFile( "TWrootfile"+options.set+"_PSET.root", "recreate" )
...
tree_vars = {"wpt":array('d',[0.]),"wmass":array('d',[0.]),"tpt":array('d',[0.]),"tmass":array('d',[0.]),"tau32":array('d',[0.]),"tau21":array('d',[0.]),"dy":array('d',[0.]),weight":array('d',[0.])}#,"nsubjets":array('d',[0.]),"sjbtag":array('d',[0.])
				...
				if len(subjetsAK8CSV)<2:
					...
					FullTop = sjbtag_cut and tau32_cut
				        if wmass_cut:
							...
							temp_variables = {"wpt":wjet.Perp(),"wmass":puppiJetMass[windexval],"tpt":tjet.Perp(),"tmass":puppiJetMass[tindexval],"tau32":tau32val,"tau21":tau21val,"dy":abs(tjet.Rapidity()-wjet.Rapidity()),"weight":weight,}#"nsubjets":nSubjets[tindexval]}
