...
Data = ROOT.TFile("rootfiles/TWanalyzerQCD_Trigger_nominal_none_PSET_"+options.cuts+".root")
...
TTmc = ROOT.TFile("rootfiles/TWanalyzerweightedttbar_Trigger_nominal_none_PSET_"+options.cuts+".root")
...
TTmcFS = TTmcFS.Rebin(rebin)
DataBE = DataBE.Rebin(rebin)
DataFS = DataFS.Rebin(rebin)
print DataFS.Integral()
DataBEl = DataBEl.Rebin(rebin)
DataBEh = DataBEh.Rebin(rebin)
...
for ihist in range(0,len(fittitles)):
	...
	QCDbkg_ARR[ihist] = QCDbkg_ARR[ihist].Rebin(rebin)
