...
parser.add_option('-r', '--rateset', metavar='F', type='string', action='store',
                  default	=	'data',
                  dest		=	'rateset',
                  help		=	'data or QCD')              
...
if jobs != 1:
	f = TFile( "TWanalyzer"+options.set+"_Trigger_"+tnameformat+"_"+options.modulesuffix +pustr+pstr+mmstr+"_job"+options.num+"of"+options.jobs+"_PSET_"+options.cuts+var+".root", "recreate" )
else:
	f = TFile( "TWanalyzer"+options.set+"_Trigger_"+tnameformat+"_"+options.modulesuffix +pustr+pstr+mmstr+"_PSET_"+options.cuts+var+".root", "recreate" )
...
TagFile1 = TFile(di+"Tagrate"+setstr+"2D.root")
