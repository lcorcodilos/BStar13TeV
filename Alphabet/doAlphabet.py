...
if options.make == 'on':
	...
	for icut in range(len(cutList)-1):
		commands1.append('python Bstar_Alphabet.py -s '+options.set+' -c '+options.cuts+' -q '+options.qcdsample+' -p '+cutList[icut]+','+cutList[icut+1])
...
totV = TH1F('FullMtwV','Alphabet estimate in full Mtw - '+options.cuts,25,50,300)
totN = TH1F('FullMtwN','Alphabet estimate in full Mtw - '+options.cuts,25,50,300)
