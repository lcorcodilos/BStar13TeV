...
class Alphabetizer:
	...
	def doRatesFlexFit(self, var, varCuts, passCuts, presel, bins, truthbins, fitFunc):
		# LC 11/14/17
		...
		self.Fit = TF1("fit", self.fitFunc, bins[0], bins[-1])
		self.FitResults = self.G.Fit(self.Fit)

		self.EG = TGraphErrors(1000)
		for i in range(1000):
			self.EG.SetPoint(i, bins[0] + i*(bins[-1]- bins[0])/1000., 0)
	...
	def MakeEstFlexFit(self, var_array, rate_var, antitag, tag, center):
	# makes an estimate in a region, based on an anti-tag region, of that variable in all dists
	# var_array - array for what we are plotting 
	# rate_var - var in which the rate was made
		fitString = CustomFit2String(rate_var,self.Fit,self.fitFunc)
		print 'Fit = ' + fitString
		self.hists_EST = []
		self.hists_EST_SUB = []
		self.hists_MSR = []
...
def CustomFit2String(var,fit,fitFunc):
	# Need to conver the fitFunc (of form '[0]+[1]*x...') to a string
	# with the actual parameters in for [0],[1], etc
	thisFitFunc = '('+fitFunc
	thisFitFunc = thisFitFunc.replace('x','('+var+')')
	pars = []
