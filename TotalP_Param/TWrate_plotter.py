...
c4 = TCanvas('c4', 'Pt fitted tagrate', 800, 500)
...
stack1 = THStack("typeb1probes", "; Probe Jet p_{T} (GeV); Events / 50(GeV)")
...
stack4 = THStack("typeb1tags", "; t-tagged Jet p_{T} (GeV); Events / 50(GeV)")
...
for j in range(0,4000):
	...
	for eta in range(0,1):
		for ifit in range(0,len(fits)):
			y[eta][ifit].append(fits[ifit].Eval(x[j]))
		BPy[eta].append(BTR.Eval(x[j]))
		BPerryh[eta].append(BTR.Eval(x[j])+sqrt(BTR_err.Eval(x[j])))
		BPerryl[eta].append(BTR.Eval(x[j])-sqrt(BTR_err.Eval(x[j])))
...
stack1.Add( probest, "Hist" )
stack1.Add( probemc, "Hist" )
stack1.Add( probedata, "Hist" )
...
stack4.Add( tagst, "Hist" )
stack4.Add( tagmc, "Hist" )
stack4.Add( tagdata, "Hist" )
...
leg2.AddEntry(probedata,"QCD","f")
leg2.AddEntry(probemc,"ttbar","f")
leg2.AddEntry(probemc,"singletop","f")
...
for ifile in range(0,len(SigFiles)):
	...
	nseta1 = SigFiles[ifile].Get("ptposttag")
...
c7.Print('plots/'+options.cuts+'/sigvsTR'+options.set+'.root', 'root')
c7.Print('plots/'+options.cuts+'/sigvsTR'+options.set+'.pdf', 'pdf')
...
for eta in range(0,1):
	print eta
	trs[eta].SetTitle(';Total p (GeV);Average t-tagging rate')
	trs[eta].GetYaxis().SetTitleOffset(0.8)
	trs[eta].SetMaximum(0.05)
	trs[eta].SetMinimum(0.0)
	trs[eta].GetXaxis().SetRangeUser(400,4000)
	trs[eta].SetStats(0)
	c4.cd()
	...
	c4.RedrawAxis()
	c4.Print('plots/'+options.cuts+'/tagrate'+options.set+'fitBP.root', 'root')
	c4.Print('plots/'+options.cuts+'/tagrate'+options.set+'fitBP.pdf', 'pdf')
	c4.Print('plots/'+options.cuts+'/tagrate'+options.set+'fitBP.png', 'png')
	...
	for ifit in range(0,len(fits)):
		...
		c4.RedrawAxis()
		c4.Print('plots/tagrate'+fittitles[ifit]+options.set+'PSET_'+options.cuts+'.root', 'root')
		c4.Print('plots/tagrate'+fittitles[ifit]+options.set+'PSET_'+options.cuts+'.pdf', 'pdf')
