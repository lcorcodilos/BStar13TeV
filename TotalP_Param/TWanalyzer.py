...
TagPlot2d= TagFile1.Get("tagrate")
	...
	if wpt_cut:
		...
		if tpt_cut:
			...
			if dy_cut:
				...
				if options.var == "kinematics":
					TopMass.Fill(topJetMass[tindexval],weight)
						...
						if Tau1[windexval] != 0 and Tau2[tindexval] != 0:
							...
							if wmass_cut:
								...
								if tau21_cut:
									...
									TTRweight = bkg_weight(tjet,TTR)
									TTRweightsigsq = bkg_weight(tjet,TTR_err)
									...
									if (eta1_cut) :
										xbin = TagPlot2d.GetXaxis().FindBin(tjet.Perp())
										ybin = TagPlot2d.GetYaxis().FindBin((tjet+wjet).M())
										tagrate2d = TagPlot2d.GetBinContent(xbin,ybin)
										tagrate2derr = TagPlot2d.GetBinError(xbin,ybin)
									...
									for spec in fillSpec:
										for ifit in range(0,len(fittitles)):
											tempweight = bkg_weight(tjet,fits[ifit])
									...
									QCDbkgMtStack.Fill(topJetMass[tindexval],TTRweight*weight*massw)
									...
									if sjbtag_cut:
										...
										if tau32_cut:
											...
											if tag==0:
												...
												MtStack.Fill(topJetMass[tindexval],weight)
