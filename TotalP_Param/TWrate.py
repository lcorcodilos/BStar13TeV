...
f.cd()
#---------------------------------------------------------------------------------------------------------------------#
ptpretag	= TH1D("ptpretag", "Total pt pre tag", 400, 0, 4000)

ptposttag	= TH1D("ptposttag", "Total pt post tag", 400, 0, 4000)


Mpre          = TH1D("Mpre",           "mass pre tag",             400,  110,  210 )
Mpre.Sumw2()

MpostFull          = TH1D("MpostFull",           "mass post tag",             400,  110,  210 )
MpostFull.Sumw2()
...
MtwwptcomparepreSB1    = TH2F("MtwwptcomparepreSB1",  "Comparison wpt and Mtw",   		400,0,2000,  140,  500,  4000 )
MtwwptcomparepostSB1    = TH2F("MtwwptcomparepostSB1",  "Comparison wpt and Mtw",   		400,0,2000,  140,  500,  4000 )

MtwwptcomparepreSB1.Sumw2()
MtwwptcomparepostSB1.Sumw2()
			...
			weighttrigdown*=weightSFt
				...
				if len(subjetsAK8CSV)<2:
			        ...
			        if SJ_csvvals != []: #added this because files with no SJ_csvvals would cause the entire thing to fail			
				        ...
				        wmass_cut = wmass[0][0]<=prunedJetMass[windexval]<wmass[0][1] or wmass[1][0]<=prunedJetMass[windexval]<wmass[1][1] 
				        ...
				        if wmass_cut:
							...
							MtwwptcomparepreSB1.Fill(tjet.P(),(tjet+wjet).M(),weight)
                        				ptpretag.Fill( tjet.P(),weight)
							Mpre.Fill(topJetMassuncorr[tindexval],weight)
                        				if FullTop :
							        MpostFull.Fill(topJetMassuncorr[tindexval],weight)
							        MtwwptcomparepostSB1.Fill(tjet.P(),(tjet+wjet).M(),weight)
                        					ptposttag.Fill( tjet.P(),weight)
                 
					            
						        temp_variables = {"wpt":wjet.Perp(),"wmass":prunedJetMass[windexval],"tpt":tjet.Perp(),"tmass":topJetMassuncorr[tindexval],"tau32":tau32val,"tau21":tau21val,"sjbtag":SJ_csvmax,"weight":weight,}#"nsubjets":nSubjets[tindexval]}
